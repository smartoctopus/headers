#if !defined(SPRINTF_H_)
#define SPRINTF_H_

/* Copied from STB-printf */
...
/* STB Printf(Public Domain Library)*/
...
#ifdef _MSC_VER
#define STBSP__NOTUSED(v) UNUSED(v)
#else
#define STBSP__NOTUSED(v) UNUSED(v)
#endif
...
#define STB_SPRINTF_DECORATE(name)                                             \
  s_##name /*define this before including if you want to change the names*/
#endif
...
#if defined(SPRINTF_IMPLEMENTATION)
...
#define STBSP__NUMSZ 512 /* big enough for e308 (with commas) or e-307 */
    ...
    case 's':
      ...
      l = stbsp__strlen_limited(s, (pr >= 0) ? cast(u32) pr : cast(u32) ~0u);
      lead[0] = 0;
      tail[0] = 0;
      pr = 0;
      dp = 0;
    ...
    case 'd': /* integer */
      ...
      if (fl & STBSP__INTMAX) {
        stbsp__int64 _i64 = va_arg(va, stbsp__int64);
        n64 = (stbsp__uint64)_i64;
        if ((f[0] != 'u') && (_i64 < 0)) {
          n64 = (stbsp__uint64)-_i64;
          fl |= STBSP__NEGATIVE;
        }
...
#if (defined(_MSC_VER) && (_MSC_VER <= 1200)) || !defined(__STDC_VERSION__)
...
/* given a float value, returns the significant bits in bits, and the position
   of the decimal point in decimal_pos.  +/-INF and NAN are specified by special
   values returned in the decimal_pos parameter. frac_digits is absolute
   normally, but if you want from first significant digits (got %g and %e), or
   in 0x80000000
*/
static stbsp__int32 stbsp__real_to_str(char const **start, stbsp__uint32 *len,
                                       char *out, stbsp__int32 *decimal_pos,
