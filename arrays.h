...
#define array_init_helper(x, _cap) do {                      \
    void** _array = cast(void**)&(x);                                   \
    ArrayHeader* _header = cast(ArrayHeader*)xmalloc(sizeof(ArrayHeader) + sizeof(*(x)) * (_cap)); \
    _header->length = 0;                                                \
...
#define array_reserve(x, new_cap) do {                                  \
    if(array_capacity(x) < (new_cap)) array_set_capacity(x, (new_cap)); \
  } while(0)
#endif

#if !defined(array_push)
#define array_push(x, item) do {                             \
    if (array_capacity(x) < array_length(x) + 1) array_grow(x, 0); \
    x[array_length(x)++] = (item);                                    \
  } while(0)
#endif

#if !defined(array_concat)
#define array_concat(x, items) do {                                     \
    if (size_of(*(x)) == size_of(*(items))) {                           \
      ...
      memcpy(&(x)[array_length(x)], (items), sizeof(*(items)) * array_length(items)); \
      array_length(x) += array_length(items);                           \
    } else printf("Concatenating arrays of different types");           \
  } while(0)
#endif

#if !defined(array_pop)
#define array_pop(x) do { if (array_length(x) > 0) ARRAY_HEADER(x)->length -= 1; else s_panic("Popping from a zero sized array"); } while(0)
#endif

#if !defined(array_clear)
#define array_clear(x) do { ARRAY_HEADER(x)->length = 0; } while(0)
#endif
...
top:
  ...
  memmove(new_header, header, sizeof(ArrayHeader) + element_size * header->length);
