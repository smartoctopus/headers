...
#define array_memcopy(dest, src, count) memset((dest), (src), (sizeof(*(dest))*(count)))
#endif

#if !defined(array_memmove)
#define array_memmove(dest, src, count) memmove((dest), (src), (sizeof(*(dest))*(count)))
#endif
...
#define s_gigabytes(x) (s_megavytes((x)) * (1024))
...
alwaysinline void* align_forward(void* ptr, usize alignment) {
  ...
  return ispow2(alignment) ? (cast(void*)((_ptr + (alignment - 1)) &~ (alignment - 1))) : NULL;
}
